							...
							<form method="post" action="">
								...
								<div class="card-footer d-flex justify-content-between align-items-center">
									...
									<div class="col-md text-right">
										<button type="button" class="btn btn-primary btn-sm btnStatus" id="btnStatus-2" data-status="2">
					...
					<div class="tab-pane fade" id="panel-2" role="tabpanel" aria-labelledby="panel-2">
						<div id="panel-2" class="panel">
							...
							<div class="card-footer d-flex justify-content-between align-items-center">
								...
								<div class="col-md text-right">
									<button type="button" class="btn btn-primary btn-sm btnStatus" id="btnStatus-3" data-status="3">
										<i class="fal fa-save"></i> Tandai progress di sini
									</button>
								</div>
							</div>
					...
					<div class="tab-pane fade" id="panel-3" role="tabpanel" aria-labelledby="panel-3">
						<div id="panel-3" class="panel">
							...
							<div class="card-footer d-flex justify-content-between align-items-center">
								...
								<div class="col-md text-right">
									<button type="button" class="btn btn-primary btn-sm btnStatus" id="btnStatus-4" data-status="4">
										<i class="fal fa-save"></i> Tandai progress di sini
									</button>
								</div>
							</div>
					...
					<div class="tab-pane fade" id="panel-4" role="tabpanel" aria-labelledby="panel-4">
						<div id="panel-4" class="panel">
							<div class="panel-container show">
								<div class="panel-tag fade show">
									<div class="d-flex align-items-center">
										<i class="fal fa-info-circle mr-1"></i>
										<div class="flex-1">
											<small class="text-danger">klik tombol di bawah untuk menandai progress pemeriksaan di sini. Set status ke "5".</small>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="card-footer d-flex justify-content-between align-items-center">
								...
								<div class="col-md text-right">
									<button type="button" class="btn btn-primary btn-sm btnStatus" id="btnStatus-5" data-status="5">
										<i class="fal fa-save"></i> Tandai progress di sini
									</button>
								</div>
							</div>
	...
	<script>
		...
		$(document).ready(function() {
					...
					},
					{
						...
						render: function(data, type, row) {
							if (data === null) {
								...
								return '<span class="btn btn-icon btn-xs btn-danger"><i class="fal fa-ban"></i></span>';
							} else {
				...
				rowCallback: function(row, data) {
					...
					$(row).on('click', function() {
						...
						if ($(this).next().hasClass('expanded-row')) {
							...
							formHTML += '<div class="mt-2"><span class="">Berkas: </span><a href="' + berkasUrl + '" download>' + data.berkas_pks + '</a></div>';
							formHTML += '<form class="mt-3" id="expandForm">';
							formHTML += '<div class="form-group">';
							formHTML += '<label for="status">Status Pemeriksaan</label>';
							...
							formHTML += '<div class="form-group">';
							formHTML += '<label for="note">Catatan Verifikasi:</label>';
							...
							formHTML += '<button class="btn btn-primary btn-xs" type="button" id="submitCheckPks" data-code="'+ data.tcode +'">Simpan</button>';
							formHTML += '</form>';
							formHTML += '</td></tr>';
			...
			$(document).on('click', '#submitCheckPks', function() {
				...
				$.ajax({
					url: "{{ route('2024.verifikator.tanam.verifPksStore', ':tcode') }}".replace(':tcode', tcode),
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						status: status,
