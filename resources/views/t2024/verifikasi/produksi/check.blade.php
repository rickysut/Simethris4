				...
				<div id="panel-1" class="panel">
								...
								<tbody>
									@foreach ($docs as $doc)
										<tr>
											...
											<td>
												<form action="{{ route("2024.verifikator.produksi.saveCheckBerkas", $ijin) }}" method="post">
													@csrf
													<input type="hidden" value="{{$doc->id}}" name="docId" id="docId">
													<select class="form-control form-control-sm saveCheckBerkas {{ is_null($doc->status) ? 'border-danger' : '' }}" name="status" id="status">
					...
					<div class="card-footer d-flex justify-content-between align-items-center">
						...
						<div class="col-md text-right">
							<form action="{{ route("2024.verifikator.produksi.markStatus", [$ijin, $tcode, "2"]) }}" method="post" enctype="multipart/form-data">
								@csrf
